  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 /* float risultato = 0.0;
	  ...
	  uint8_t msg_t[35] = {'\0'};
	  uint8_t msg_r[9] = {'\0'};
	  sprintf(msg_t, "Inserisci angolo: \r",sizeof(msg_r)-1);
	  Test_COM_Tx(msg_t);

	  Test_COM_Rx(msg_r, sizeof(msg_r));

	  sprintf(msg_t, "Angolo inserito: \r");
...
*/
	  /*
	  risultato = servo_motor(appoggio);
	  printf("Valore CCR: %f \r\n",risultato);
...
void Test_COM_Tx(uint8_t* msg)
{
	...
	uint8_t eo1 [2]= "\r\n";
	HAL_UART_Transmit(&huart3, eo1, sizeof(eo1), timeout);
}
